...
public class PersonRequestDTO {
    ...
    @Pattern(regexp = "^[\\p{L} .'-]+$", message ="Debe ingresar un nombre sin números." )
    private String name;

    @Size(min = 2,message =  "Debe ingresar un apellido mayor a 2 caracteres.")
    @Pattern(regexp = "^[\\p{L} .'-]+$", message ="Debe ingresar un apellido sin números." )
    private String lastName;

    @NotEmpty(message ="El linkedin no puede estar vacio")
    private String linkedin;

    @Pattern(regexp = "^[\\p{L} .'-]+$", message ="Debe ingresar el nombre del recruiter sin números." )
